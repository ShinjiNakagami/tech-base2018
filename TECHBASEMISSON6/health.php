...
$sql= "CREATE TABLE keijibann_health7"
." ("
...
if(!empty($_POST['comment']) or !empty($_FILES['up_image']) or !empty($_FILES['up_video']) ){
  ...
  $imsave_health = 'imagehealth/'.$im_name;
  $video_name = $_FILES['up_video']['name'];
  $video_tmpname = $_FILES['up_video']['tmp_name'];
  $videosave_health = 'videohealth/'.$video_name;
  $sql = $pdo -> prepare("INSERT INTO keijibann_health7( name, comment, icon_add, im_add, video_add, now) VALUES( :name, :comment, :icon_add, :im_add, :video_add,  :now)");
  $sql-> bindParam(':name', $name, PDO::PARAM_STR);
  $sql-> bindParam(':comment', $comment, PDO::PARAM_STR);
  $sql-> bindParam(':icon_add', $icon, PDO::PARAM_STR);
  $sql-> bindParam(':im_add', $imsave_health, PDO::PARAM_STR);
  $sql-> bindParam(':video_add', $videosave_health, PDO::PARAM_STR);
  $sql-> bindParam(':now', $now, PDO::PARAM_STR);
  $sql-> execute();
  if(is_uploaded_file($im_tmpname)){
    if(move_uploaded_file($im_tmpname, $imsave_health)){
      //下二行はチェック用
  ...
  if(is_uploaded_file($video_tmpname)){
    if(move_uploaded_file($video_tmpname, $videosave_health)){
      //下二行はチェック用
      //echo $filename."をアップロードしました。";
      //echo '<p><img src="'.$filesave.'"/>';*/
    }
...
$sql = 'select * from keijibann_health7 order by id desc';
...
foreach($results as $row){
  ...
  <?php
    if( basename($row['im_add']) != 'imagehealth' ){
    ?>
    <p><img src="<?php echo $row['im_add'];?>" class="left-imageshow"/></p>
      <?php
    }
    if( basename($row['video_add']) != 'videohealth' ){
      ?>
