...
img.right-imageshow{
  ...
  <header id="header">
      ...
      <br>
      <form action="wear.php" method="POST" enctype="multipart/form-data">
    ...
    <table>
      <tbody>
        <form action="wear.php" method="POST">
    <tr>
    <td>
...
$sql= "CREATE TABLE keijibann_wear7"
." ("
...
if(!empty($_POST['comment']) or !empty($_FILES['up_image']) or !empty($_FILES['up_video']) ){
  ...
  $imsave_wear = 'imagewear/'.$im_name;
  $video_name = $_FILES['up_video']['name'];
  $video_tmpname = $_FILES['up_video']['tmp_name'];
  $videosave_wear = 'videowear/'.$video_name;
  $sql = $pdo -> prepare("INSERT INTO keijibann_wear7( name, comment, icon_add, im_add, video_add, now) VALUES( :name, :comment, :icon_add, :im_add, :video_add,  :now)");
  $sql-> bindParam(':name', $name, PDO::PARAM_STR);
  $sql-> bindParam(':comment', $comment, PDO::PARAM_STR);
  $sql-> bindParam(':icon_add', $icon, PDO::PARAM_STR);
  $sql-> bindParam(':im_add', $imsave_wear, PDO::PARAM_STR);
  $sql-> bindParam(':video_add', $videosave_wear, PDO::PARAM_STR);
  $sql-> bindParam(':now', $now, PDO::PARAM_STR);
  $sql-> execute();
  if(is_uploaded_file($im_tmpname)){
    if(move_uploaded_file($im_tmpname, $imsave_wear)){
      //下二行はチェック用
  ...
  if(is_uploaded_file($video_tmpname)){
    if(move_uploaded_file($video_tmpname, $videosave_wear)){
      //下二行はチェック用
      //echo $filename."をアップロードしました。";
      //echo '<p><img src="'.$filesave.'"/>';*/
    }
...
$sql = 'select * from keijibann_wear7 order by id desc';
...
foreach($results as $row){
  ...
  <?php
    if( basename($row['im_add']) != 'imagewear' ){
    ?>
    <p><img src="<?php echo $row['im_add'];?>" class="left-imageshow"/></p>
      <?php
    }
    if( basename($row['video_add']) != 'videowear' ){
      ?>
