<?php #Mysqlへの接続を行うid, name, comment, time, password, d_num, d_pass, e_num
...
#insertでデータ入力.名前とコメントが入力されている場合
if ( !empty($_POST['name']) and !empty($_POST['comment'])){
if (empty($_POST['e_num'])){#こちらは普通の投稿の時
$name = $_POST['name'];
...
$show_e_num = "";
$show_password1 = "";
$sql = $pdo -> prepare("INSERT INTO mission4test4( name, comment, time, password1, d_num, d_pass, e_num) VALUES( :name, :comment, :time, :password1, :d_num, :d_pass, :e_num)");
...
$sql-> execute();
}elseif ( !empty($_POST['e_num']) ){#編集番号が入力されているとき.これが実際に編集を行うときの処理
$name = $_POST['name'];
$comment = $_POST['comment'];
$password1 = $_POST['password1'];
$e_num = $_POST['e_num'];
...
#編集機能編集モードに入る。画面の入力フォームにDBの内容を記入する
